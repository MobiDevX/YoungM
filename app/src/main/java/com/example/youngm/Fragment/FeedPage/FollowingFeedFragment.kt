...
class FollowingFeedFragment : Fragment(),
    ...
    override fun onActivityCreated(savedInstanceState: Bundle?) {
        ...
        recyclerFollowingFeed.adapter = adapter
        recyclerFollowingFeed.layoutManager = LinearLayoutManager(context)
        musicDataViewModel = MusicDataViewModel(context, "FollowingFeedFragment")
        postsList = ArrayList()

        progressBar = progressBarFollowing
        updateData()

        if (swipeRefreshFollowing != null){
...
//            updateData()
    ...
    fun loadPosts(){
        try {
            ...
            recyclerFollowingFeed.visibility = View.INVISIBLE
            val url = URLData.MAIN_URL + URLData.GET_FOLLOWING_POSTS
            ...
            val request =
                Request.Builder().url(url).header("Authorization", "Token $TOKEN").post(requestBody)
                    .build()
                    ...
                    override fun onResponse(call: Call, response: Response) {
                        if (response.code == 200) {
                            ...
                            val refreshJob: Job = GlobalScope.launch(Dispatchers.Main) {
                                ...
                                recyclerFollowingFeed.visibility = View.VISIBLE
                            }
                            refreshJob.start()
